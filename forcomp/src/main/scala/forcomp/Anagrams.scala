...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    ...
    case (char, freq)::_ =>   {
      ...
      } yield if (newFreq == 0) rest else (char, newFreq) :: rest
    }.toList
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    def subtractTerm(terms: Map[Char, Int], term: (Char, Int)): Map[Char, Int] = {
      val (char, freq) = term
      val newFreq = terms(char) - freq
      if (newFreq == 0) terms - char else terms.updated(char, newFreq)
    }

    (y.toMap foldLeft x.toMap)(subtractTerm).toList.sorted
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def encode(occurrences: Occurrences): List[Sentence] = occurrences match {
      case Nil => List(Nil)
      case _ => for {
        subOcc <- combinations(occurrences)
        word <- dictionaryByOccurrences(subOcc)
        rest <- encode(subtract(occurrences, subOcc))
      } yield word :: rest
    }
