...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val classificationMap = means.par.map(m => m -> GenSeq[Point]()).toMap
    classificationMap ++ points.par.groupBy(findClosest(_, means))
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    oldMeans.zipWithIndex.par.map { case (om, index) => (findAverage(om, classified(om)), index) }.seq.sortWith(_._2 < _._2).map { case (point, _) => point }
  }

  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    oldMeans.zip(newMeans).par.forall { case (om, nm) => om.squareDistance(nm)  <= eta }
  }
