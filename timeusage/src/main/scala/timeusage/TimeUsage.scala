...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType =
    new StructType((StructField(columnNames.head, StringType, false) ::  columnNames.tail.map(StructField(_, DoubleType, false) )).toArray)
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val (primaryNeedActivities, remainingActivities1) = columnNames.partition(cn => Seq("t01", "t03", "t11", "t1801", "t1803").exists(cn.startsWith))
    val (workingActivities, remainingActivities2) = remainingActivities1.partition(cn => Seq("t05", "t1805").exists(cn.startsWith))
    val otherActivities = remainingActivities2.filter(cn => Seq("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18").exists(cn.startsWith))

    (primaryNeedActivities.map(column), workingActivities.map(column), otherActivities.map(column))
  }
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column = when($"telfs" >= 1 and $"telfs" < 3.0, "working").otherwise("not working").as("working")
    val sexProjection: Column = when($"tesex" === 1.0, "male").otherwise("female").as("sex")
    val ageProjection: Column = when($"teage" >= 15.0 and $"teage" <= 22.0, "young").when($"teage" >= 23.0 and $"teage" <= 55.0, "active").otherwise("elder").as("age")
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] = {
    ...
    /* NOTE: seems like over kill given that the columns in the data frame are already typed correctly
