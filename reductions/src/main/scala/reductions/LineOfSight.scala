...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = input.zipWithIndex.foreach {
      case (_, 0) => output(0) = 0
      case (xs, i) => output(i) = max(output(i - 1), xs / i)
    }
   ...
   */
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = input.zipWithIndex.slice(from, until).map { case(xs, i) => if (i == 0) 0f else xs / i }.max
   ...
   */
  def downsweepSequential(input: Array[Float], output: Array[Float], startingAngle: Float, from: Int, until: Int): Unit = if (from < until) {
