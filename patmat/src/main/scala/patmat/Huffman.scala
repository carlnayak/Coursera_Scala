...
object Huffman {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case n1 :: n2 :: ns => (makeCodeTree(n1, n2) :: ns).sortBy(ct => weight(ct))
    case _ => trees
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def traverse(subTree: CodeTree)(bits: List[Bit]): List[Char] = {
      subTree match {
        case Leaf(char, _) => if (bits.isEmpty) char :: Nil else char :: traverse(tree)(bits)
        case Fork(left, right, _, _) => traverse(if (bits.head == 0) left else right)(bits.tail)
      }
    }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def traverse(subTree: CodeTree)(char: Char): List[Bit] = {
      subTree match {
        case Leaf(_, _) => List()
        case Fork(left, right, _, _) => if (chars(left).contains(char)) 0 :: traverse(left)(char) else 1 :: traverse(right)(char)
      }
    }
  ...
  def convert(tree: CodeTree): CodeTable = {
    def encode(tree: CodeTree): CodeTable = {
      tree match {
        case Leaf(char, _) => List((char, List()))
        case Fork(left, right, _, _) => mergeCodeTables(encode(left), encode(right))
      }
    }
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
    def prepend(bit: Bit)(pair: (Char, List[Bit])): (Char, List[Bit]) = (pair._1, bit :: pair._2)
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def encode(table: CodeTable)(text: List[Char]): List[Bit] = {
      text match {
        case List() => List()
        case t1 :: ts => codeBits(table)(t1) ::: encode(table)(ts)
      }
    }
