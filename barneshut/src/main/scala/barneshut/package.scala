...
package object barneshut {
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: Seq[Body]) extends Quad {
    ...
    val massX = bodies.foldLeft(0f)((sum, b) => sum + (b.mass * b.x)) / mass
    val massY = bodies.foldLeft(0f)((sum, b) => sum + (b.mass * b.y)) / mass
    val total: Int = bodies.length

    def insert(b: Body): Quad = if (size > minimumSize) {
      val sizeByTwo  = size / 2
      val sizeByFour = size / 4
      val westX = centerX - sizeByFour
      val eastX = centerX + sizeByFour
      val northY = centerY - sizeByFour
      val southY = centerY + sizeByFour

      val nw = Empty(westX, northY, sizeByTwo)
      val ne = Empty(eastX, northY, sizeByTwo)
      val sw = Empty(westX, southY, sizeByTwo)
      val se = Empty(eastX, southY, sizeByTwo)
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def combine(that: SectorMatrix): SectorMatrix = {
      // this.matrix.zip(that.matrix).map { case (m1, m2) => m1.combine(m2) }
